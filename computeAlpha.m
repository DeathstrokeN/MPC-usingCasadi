...
for i= 1:max_iter
    ...
    for j = 1:4
        % Optimize using fmincon here:
        % [x2,val] = fmincon( ??? );
        % Arguments for fmincon:
        % fmincon(FUN,X0,A,B,Aeq,Beq,LB,UB,nonlcon,opt)
        % fun: objective function to be MINIMIZED
        % X0: Set x2_init(:,j)
        % A,B,Aeq,Beq,LB,UB: Linear (in)equality constraints and lower and
        %                    upper bound. Set [] for each of them.
        % nonlcon: Nonlinear (in) equality constraints. 
        %          Set @(x)nonlcon(x,P,alpha) and fill in the function
        %          nonlcon below.
        % opt: Additional options for fmincon. Set opt.
